    ...
    private randomizeBuyRestriction(assortItemTable)
    {
        ...
        if (!realismDetected) // If realism is not detected, continue, else do nothing
        {
            for (const item in assortItemTable)
            {
                ...
                const newRestriction = Math.round(randomUtil.randInt(Math.floor((oldRestriction * 0.5)), oldRestriction));
        ...
        if (!realismDetected) // If realism is not detected, continue, else do nothing
        {
            for (const item in assortItemTable)
            {
                ...
                else
                {
                    ...
                    const newStock = randomUtil.randInt(2, Math.floor((originalStock*0.5)));
